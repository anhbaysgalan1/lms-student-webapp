...
class WatchVideo extends Component {
  ...
  handleClick(index, video) {
    const { history, getCurrentVideoAction, clearCurrentVideoAction } = this.props;
    this.setState({
      videoActive: index,
    });
    ...
    if (videoInPlaylistReducer) {
      ...
      return (
        _.map(listVideo, (video, index) => (
          <div key={video._id} className="mt-3 related-video" style={{ display: videoActive === index || currentVideoReducer.videoId === video.videoId ? 'none' : 'block' }} onClick={() => this.handleClick(index, video)} onKeyDown={() => {}} role="presentation">
